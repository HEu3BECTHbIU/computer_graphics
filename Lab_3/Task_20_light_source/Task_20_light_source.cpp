...
public:
    ...
    virtual void RenderSceneCB() // функция отрисовки
    {
        ...
        PointLight pl[3]; // три точечных источника света разных цветов и в разных позициях
        pl[0].DiffuseIntensity = 0.5f;
        pl[0].Color = Vector3f(1.0f, 0.0f, 0.0f); // красный
        pl[0].Position = Vector3f(sinf(m_scale / 10) * 10, 1.0f, cosf(m_scale / 10) * 10);
        pl[0].Attenuation.Linear = 0.1f;

        pl[1].DiffuseIntensity = 0.5f;
        pl[1].Color = Vector3f(0.0f, 1.0f, 0.0f);
        pl[1].Position = Vector3f(sinf(m_scale / 10 + 2.1f) * 10, 1.0f, cosf(m_scale / 10 + 2.1f) * 10);
        pl[1].Attenuation.Linear = 0.1f;

        pl[2].DiffuseIntensity = 0.5f;
        pl[2].Color = Vector3f(0.0f, 0.0f, 1.0f);
        pl[2].Position = Vector3f(sinf(m_scale / 10 + 4.2f) * 10, 1.0f, cosf(m_scale / 10 + 4.2f) * 10);
