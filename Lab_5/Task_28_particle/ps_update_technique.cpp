...
// VS метода обновления частиц. все что он делает - передает вершины в GS
...
// геометричский шейдер. Все атрибуты мы получим из VS, они же и выйдут в буфер transform feedback buffer
...
// обычная инициализация шейдеров
bool PSUpdateTechnique::Init()
{
    ...
    Varyings[3] = "Age1";
    // Для указания атрибутов, которые попадут в буфер, вызываем
    // Когда transform feedback включен, эти атрибуты будут записаны в буфер по - вершинно
    glTransformFeedbackVaryings(m_shaderProg, 4, Varyings, GL_INTERLEAVED_ATTRIBS);

    if (!Finalize()) {
        return false;
    }
    // получение юниформ переменных
...
// функции, передающие соответствующие значения в шейдер через юниформ переменные
